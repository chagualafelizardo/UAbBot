...
#     def _format_faq_response(self, faq_match: Dict) -> List[Dict]:
#         """Formata resposta para FAQ com botões interativos e mantém informações financeiras"""
#         response_parts = [
#             {
#                 'text': f"❓ **Pergunta encontrada em {faq_match['filename']}:**\n{faq_match['question']}",
#                 'metadata': {
...
#         # Mantém a seção financeira da versão original
...
#         # Gera perguntas relacionadas (versão melhorada)
#         related_questions = self._generate_related_questions(faq_match)
        
#         if related_questions:
#             # Cabeçalho adaptado da versão original
#             response_parts.append({
#                 'text': "\n🔍 Talvez você queira saber também:",
#                 'metadata': {'type_speed': 20}
#             })
            
#             # Botões da nova versão com ícone da versão original
#             buttons = []
#             for question in related_questions:
#                 buttons.append({
#                     'title': f"• {question}",  # Mantém o marcador visual
#                     'payload': f"/ask_follow_up{{\"question\":\"{question}\"}}"  # Intent mais descritiva
#                 })
            
#             response_parts.append({
#                 'text': "Clique em uma opção para saber mais:",
#                 'metadata': {
#                     'buttons': buttons,
#                     'suggested_questions': related_questions
#                 }
...
#         # Feedback (combina ambas as versões)
#         response_parts.append({
#             'text': "\nEsta informação resolveu sua dúvida?",
#             'metadata': {
#                 'response_part': 'confirmation',
...
# # Classe para retornar botoes de questoes sugeridas pelo bot ao usuario    
# class ActionAnswerSuggestedQuestion(Action):
#     def name(self) -> Text:
#         return "action_answer_suggested_question"
...
#         # Extrai a pergunta do tracker
#         question = tracker.latest_message.get("text", "")
        
#         try:
#             # Se veio de um botão, extrai do metadata
#             metadata = tracker.latest_message.get("metadata", {})
#             if "question" in metadata:
#                 question = metadata["question"]
            
#             # Usa a instância existente de ActionSmartSearch
#             search_action = ActionSmartSearch()
            
#             # Realiza a busca
#             docs = search_action._semantic_search_rag(question, top_k=search_action.top_k)
#             faq_match = search_action._find_best_faq_match(question, docs)
            
#             if faq_match:
#                 response_parts = search_action._format_faq_response(faq_match)
#                 for part in response_parts:
#                     dispatcher.utter_message(
#                         text=part['text'],
#                         buttons=part.get('metadata', {}).get('buttons', []),
#                         metadata=part.get('metadata', {})
#                     )
#             else:
#                 dispatcher.utter_message(
#                     text=f"Resposta para: {question}\n\nNão encontrei informações adicionais sobre este tópico.",
#                     buttons=[
#                         {
#                             'title': '🔄 Tentar outra pergunta',
...
#         except Exception as e:
#             logger.error(f"Erro ao responder pergunta sugerida: {str(e)}")
#             dispatcher.utter_message(
#                 text="Ocorreu um erro ao processar sua pergunta. Por favor, tente novamente."
#             )
